			...
			return this
					.each(function() {
						...
						function keypressEvent(e) {
							...
							if (k == undefined)
								return false; // needed to handle an IE
												// "special" event
							if (k < 48 || k > 57) { // any key except the
													// numbers 0-9
								if (k == 45) { // -(minus) key
									markAsDirty();
						...
						function canInputMoreNumbers(element) {
							var reachedMaxLenght = (element.val().length >= element
									.attr('maxlength') && element
									.attr('maxlength') >= 0);
							var selection = getInputSelection(element.get(0));
							var start = selection.start;
							var end = selection.end;
							var hasNumberSelected = (selection.start != selection.end && element
						...
						function keydownEvent(e) {
							...
							if (k == 8) { // backspace key
								...
								return true;
							} else if (k == 46 || k == 63272) { // delete key
																// (with special
																// case for
																// safari)
								preventDefault(e);
						...
						function focusEvent(e) {
							...
							if (this.createTextRange) {
								...
								textRange.collapse(false); // set the cursor at
															// the end of the
															// input
								textRange.select();
							}
