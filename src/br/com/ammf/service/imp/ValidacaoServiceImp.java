...
public class ValidacaoServiceImp implements ValidacaoService {
	...
	public boolean depoimento(Depoimento depoimento, Result result) {
		...
		if (depoimento.getEmail() == null || depoimento.getEmail().isEmpty()) {
			...
			result.include("emailEmBranco",
					"O email est&aacute; com formato inv&aacute;lido<br/>");
			validado = false;
		}

		if (depoimento.getConteudo() == null
				|| depoimento.getConteudo().isEmpty()) {
			result.include("textoEmBranco",
					"O depoimento deve ser informado<br/>");
			validado = false;
		} else if (depoimento.getConteudo().length() > 500) {
			result.include("textoEmBranco",
					"O depoimento deve ter at&eacute; 500 caracteres<br/>");
			validado = false;
		}
		...
		if (!validado) {
			...
			/*
			 * flag para a pagina depoimentoCliente.jsp fechar a listagem dos
			 * depoimentos cadastrados e mostrar o form de novo depoimento.
			 */
	...
	public boolean mensagem(Mensagem mensagem, Result result) {
		...
		if (mensagem.getConteudo() == null || mensagem.getConteudo().isEmpty()) {
			result.include("conteudoEmBranco",
					"A mensagem deve ser informada<br/>");
			validado = false;
		} else if (mensagem.getConteudo().length() > 500) {
			result.include("conteudoEmBranco",
					"O mensagem deve ter at&eacute; 500 caracteres<br/>");
			validado = false;
		}
	...
	public boolean blog(Texto texto, Result result) {
		...
		if (texto.getTitulo() == null || texto.getTitulo().isEmpty()) {
			result.include("tituloEmBranco",
					"O t&iacute;tulo deve ser informado<br/>");
			validado = false;
		}
		if (texto.getAutor() == null || texto.getAutor().isEmpty()) {
			result.include("autorEmBranco",
					"O nome do autor deve ser informado<br/>");
			validado = false;
		}
		if (texto.getConteudo() == null || texto.getConteudo().isEmpty()) {
			result.include("conteudoEmBranco",
					"O conte&uacute;do do texto deve ser informado");
			validado = false;
		} else if (texto.getConteudo().length() > 20000) {
			result.include(
					"conteudoEmBranco",
					"O conte&uacute;do do texto tem "
							+ texto.getConteudo().length()
							+ " caracteres. Deve possuir no m&aacute;ximo 20000 caracteres");
			validado = false;
		}
		if (!validado) {
	...
	public boolean pessoa(boolean aceiteCadastro, Pessoa pessoa, Result result) {
		...
		if (pessoa.getEmail() == null || pessoa.getEmail().isEmpty()) {
			...
			emailCadastrado = pessoaRepository.jaEstaCadastrada(pessoa
					.getEmail());
			if (emailCadastrado) {
				emailRemovido = pessoaRepository.jaFoiRemovida(pessoa
						.getEmail());
				if (emailRemovido) {
					result.include(
							"emailEmBranco",
							"O email "
									+ pessoa.getEmail()
									+ " j&aacute; est&aacute; cadastrado neste site, porém está inativo. Caso deseje ativá-lo acesse a página de contato e envie uma mensagem solicitando a ativação<br/>");
					...
					result.include(
							...
							"O email "
									+ pessoa.getEmail()
									+ " j&aacute; est&aacute; cadastrado neste site<br/>");
					result.include("emailJaCadastrado", true);
					validada = false;
				}
			}
		...
		if (!aceiteCadastro && !emailCadastrado) {
			result.include("aceiteEmBranco",
					"O termo de aceite de cadastro deve ser confirmado<br/>");
			validada = false;
		}
	...
	public boolean usuario(Usuario usuario, Result result) {
		 ...
		 * result.include("usuarioUuidBranco",
		 * "N&atilde;o foi poss&iacute;vel definir o uuid do usu&aacute;rio");
		 * resultado = false; }
		 */
		...
		if (usuario.getEmail() == null || usuario.getEmail().isEmpty()) {
			...
			result.include("usuarioErroEmail",
					"O email deve possuir um formato v&aacute;lido");
			resultado = false;
		}

		if (usuario.getEmailNotificacao() == null
				|| usuario.getEmailNotificacao().isEmpty()) {
			result.include("usuarioErroEmailNotificacao",
					"O email para notifica&ccedil;&atilde;o deve ser informado");
			resultado = false;
		} else if (!ehEmailValido(usuario.getEmailNotificacao())) {
			result.include("usuarioErroEmailNotificacao",
					"O email para notifica&ccedil;&atilde;o deve possuir um formato v&aacute;lido");
			resultado = false;
		}
		 ...
		 * result.include("usuarioErroEmail",
		 * "O email deve deve ser do Gmail (seu_email@gmail.com)"); resultado =
		 * false; }
		 */
	...
	@Override
	public boolean cadastrarLivro(UploadedFile imagemLivro, Livro livro,
			Result result) {
		boolean validado = verificarCamposPreenchidos(imagemLivro, livro,
				result);
		if (!validado) {
	...
	@Override
	public boolean atualizarLivro(UploadedFile imagemLivro, Livro livro,
			Result result) {
		boolean validado = verificarCamposPreenchidos(imagemLivro, livro,
				result);
	...
	private boolean verificarCamposPreenchidos(UploadedFile imagemLivro,
			Livro livro, Result result) {
		boolean validado = true;
		...
		if (livro.getSubtitulo() == null || livro.getSubtitulo().isEmpty()) {
			result.include("subtituloEmBranco",
					"O subt&iacute;tulo deve ser informado<br/>");
			validado = false;
		}

		if (livro.getPaginas() == 0) {
			result.include("paginaEmBranco",
					"O n&uacute;mero de p&aacute;ginas deve ser informado<br/>");
			validado = false;
		}

		if (livro.getEdicao() == 0) {
			result.include("edicaoEmBranco",
					"O n&uacute;mero da edi&ccedil;&atilde;o deve ser informado<br/>");
			validado = false;
		}

		if (livro.getAno() == 0) {
			validado = setMsgErroAno(result,
					"O ano de lan&ccedil;amento deve ser informado<br/>");
		} else {
			if (livro.getAno() < 1500) {
				validado = setMsgErroAno(result,
						"O ano de lan&ccedil;amento deve ser superior a ano 1499<br/>");
			}

			if (livro.getAno() > DataUtils.getAnoCorrente()) {
				validado = setMsgErroAno(result,
						"O ano de lan&ccedil;amento n&atilde;o pode ser posterior ao ano corrente<br/>");
			}
		}

		if (livro.getPreco() == null) {
			result.include("precoEmBranco",
					"O pre&ccedil;o deve ser informado<br/>");
			validado = false;
		}

		if (livro.getLinkVenda() == null || livro.getLinkVenda().isEmpty()) {
			result.include("linkEmBranco",
					"O link de venda do produto deve ser informado<br/>");
			validado = false;
			...
			} else if (!livro.getLinkVenda().startsWith("http")
					&& !livro.getLinkVenda().startsWith("https")) {
				result.include("linkEmBranco",
						"O link de venda do produto deve come&ccedil;ar com '<b>http://www.</b>'<br/>");
		...
		if (imagemLivro != null) {
			if (!imagemLivro.getContentType().startsWith("image")) {
				result.include(
						"fotoInvalida",
						"O arquivo para a capa do livro deve ser um arquivo de foto no formato .gif, .jpg ou .png.");
				validado = false;
			}
		}
		return validado;
	...
	@Override
	public boolean novaResenha(UploadedFile imagemResenha, Resenha resenha,
			Result result) {
	...
	@Override
	public boolean atualizarResenha(UploadedFile novaImagemResenha,
			Resenha resenha, Result result) {
	...
	private boolean resenha(UploadedFile imagemResenha, Resenha resenha,
			Result result) {
		boolean validado = true;
		...
		if (resenha.getDescricao() == null || resenha.getDescricao().isEmpty()) {
			result.include("descricaoEmBranco",
					"O coment&aacute;rio deve ser informado<br/>");
			validado = false;
		} else if (resenha.getDescricao().length() > 20000) {
			result.include(
					"descricaoEmBranco",
					"O coment&aacute;rio possui "
							+ resenha.getDescricao().length()
							+ " caracteres. Deve ter no m&aacute;ximo 20000 caracteres<br/>");
			validado = false;
		}
		...
		if (imagemResenha != null) {
			if (!imagemResenha.getContentType().startsWith("image")) {
				result.include(
						"fotoInvalida",
						"O arquivo para a apresenta&ccedil;&atilde;o da resenha deve ser um arquivo de foto no formato .gif, .jpg ou .png.");
				validado = false;
			}
		}
	...
	public boolean cadastrarLink(Link link, Result result) {
		...
		if (link.getTitulo() == null || link.getTitulo().isEmpty()) {
			result.include("linkEmBranco",
					"O t&iacute;tulo para o link deve ser informado<br/>");
			validado = false;
		}

		if (link.getUrl() == null || link.getUrl().isEmpty()) {
			result.include("urlEmBranco",
					"A URL do link deve ser informada<br/>");
			validado = false;
		} else {
			...
			if (link.getUrl().length() > 500) {
				result.include("urlEmBranco",
						"A URL do link excedeu o tamanho limite de 500 caracteres.<br/>");
				validado = false;
			}
		}
		...
		if (link.getDescricao() == null || link.getDescricao().isEmpty()) {
			result.include("descricaoEmBranco",
					"A descri&ccedil;&atilde;o do link deve ser informada<br/>");
			validado = false;
		} else if (link.getDescricao().length() > 500) {
			result.include(
					"descricaoEmBranco",
					"A descri&ccedil;&atilde;o do link excedeu o tamanho limite de 500 caracteres.<br/>");
			validado = false;
		}
	...
	public boolean cadastrarFaq(Faq faq, Result result) {
		...
		if (faq.getEmail() == null || faq.getEmail().isEmpty()) {
			result.include("emailEmBranco",
					"O seu email deve ser informado<br/>");
			validado = false;
		} else if (!ehEmailValido(faq.getEmail())) {
			result.include("emailEmBranco",
					"O seu email est&aacute; com formato inv&aacute;lido<br/>");
			validado = false;
		}

		if (faq.getPergunta() == null || faq.getPergunta().isEmpty()) {
			result.include("perguntaEmBranco",
					"A sua pergunta deve ser informada<br/>");
			validado = false;
		}
	...
	@Override
	public boolean cadastrarComentario(Comentario comentario, Local local,
			Result result) {
		boolean validado = true;
		...
		if (comentario.getConteudo() == null
				|| comentario.getConteudo().isEmpty()) {
			result.include("conteudoEmBranco",
					"O coment&aacute;rio deve ser informado<br/>");
			validado = false;
		}
	...
	public boolean cadastrarEvento(Evento evento, Result result) {
		...
		if (evento.getData() == null) {
			result.include("dataEmBranco",
					"A data da constelação deve ser informada<br>");
			validado = false;
		}

		if (evento.getLocal() == null || evento.getLocal().isEmpty()) {
			result.include("localEmBranco",
					"O endereço do local da constelação deve ser informado<br>");
			validado = false;
		}

		if (evento.getValorIngresso() == null) {
			result.include("valorIngressoEmBranco",
					"O valor do ingresso deve ser informado e conter apenas dígitos<br>");
			validado = false;
		}

		if (evento.getValorParticipacao() == null) {
			result.include("valorConstelacaoEmBranco",
					"O valor para Constelar deve ser informado e conter apenas dígitos<br>");
			validado = false;
		}
	...
	@Override
	public String cadastrarComentario(String uuidTextoBlog,
			String comentarioNome, String comentarioEmail,
	...
	@Override
	public void verificarCamposPreenchidos(Texto texto, Local local,
			Result result) {
		if (texto.getAutor() == null || texto.getAutor().isEmpty()) {
		...
		if (texto.getConteudo() == null || texto.getConteudo().isEmpty()) {
			...
			case 0:
				if (texto.getConteudo().length() > 600) {
					result.include(
							"mensagemAviso",
							"O texto da p&aacute;gina '?1' continha ?2 caracteres e foi recortado em 600 caracteres para ser atualizado"
									.replace("?1", local.toString()).replace(
											"?2",
											String.valueOf(texto.getConteudo()
													.length())));
			...
			default:
				if (texto.getConteudo().length() > 2999) {
					result.include(
							"mensagemAviso",
							"O texto da p&aacute;gina '?1' continha ?2 caracteres e foi recortado em 3000 caracteres para ser atualizado"
									.replace("?1", local.toString()).replace(
											"?2",
											String.valueOf(texto.getConteudo()
													.length())));
	...
	/*
	 * private boolean ehGmail(String email) { return
	...
	@Override
	public boolean ehEmailValido(String email, Result result) {
